...
Trigger OnUserCreate
  Method sync-with-marketing-service`,
...
System "E-commerce"
  ...
  Requirement "Payments"
    must: be-secure, support-stripe
  Flow "Checkout"
    Page:Cart -> API:Payment => Model:Order`
}

/**
 * Generates the core NEXUS notation reference based on project DNA.
 * This is the primary tool for inducing NEXUS context into AI models.
 * 
 * @param config - Project DNA configuration (frameworks, modules, tokens).
 * @returns A formatted string containing the grammar reference and examples.
 */
export function buildPrompt(config: Partial<NexusConfig>): string {
  ...
  activeModules.forEach((mod: string) => {
    ...
    if (mod === 'backend') {
      ['Model', 'Controller', 'Middleware', 'Service', 'Endpoint', 'Worker', 'Queue', 'CronJob', 'Router', 'Index', 'Trigger', 'Procedure'].forEach(o => {
    ...
    if (mod === 'sdd') {
      ['System', 'Actor', 'Flow', 'Requirement'].forEach(o => {
        if (!orchestrators.includes(o)) orchestrators.push(o)
      })
    }
  })
...
/**
 * Build a provider-optimized system prompt for AI integration.
 * Combines a provider-specific instruction with the full NEXUS reference.
 * 
 * @param config - Project DNA configuration.
 * @param provider - The target AI model provider (default: 'claude').
 * @returns A complete system prompt ready for API consumption.
 */
